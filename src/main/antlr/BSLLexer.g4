...
PREPROC_DELETE
    : '#' [ \t]*
    ('УДАЛЕНИЕ' | 'DELETE') -> pushMode(PREPROC_DELETE_MODE), channel(PREPROC_DELETE_CHANNEL);
PREPROC_INSERT
    : '#' [ \t]*
    ('ВСТАВКА' | 'INSERT') -> channel(HIDDEN);
PREPROC_ENDINSERT
    : '#' [ \t]*
    ('КОНЕЦВСТАВКИ' | 'ENDINSERT') -> channel(HIDDEN);
...
PREPROC_NOT_KEYWORD:'НЕ' | 'NOT';
PREPROC_OR_KEYWORD: 'ИЛИ' | 'OR';
PREPROC_AND_KEYWORD: 'И' | 'AND';

PREPROC_IF_KEYWORD: 'ЕСЛИ' | 'IF';
PREPROC_THEN_KEYWORD: 'ТОГДА' | 'THEN';
PREPROC_ELSIF_KEYWORD: 'ИНАЧЕЕСЛИ' | 'ELSIF';
PREPROC_ENDIF_KEYWORD: 'КОНЕЦЕСЛИ' | 'ENDIF';
PREPROC_ELSE_KEYWORD: 'ИНАЧЕ' | 'ELSE';
...
ANNOTATION_ATSERVERNOCONTEXT_SYMBOL
    : ('НАСЕРВЕРЕБЕЗКОНТЕКСТА' | 'ATSERVERNOCONTEXT') -> popMode
    ;

ANNOTATION_ATCLIENTATSERVERNOCONTEXT_SYMBOL
    : ('НАКЛИЕНТЕНАСЕРВЕРЕБЕЗКОНТЕКСТА' | 'ATCLIENTATSERVERNOCONTEXT') -> popMode
    ;

ANNOTATION_ATCLIENTATSERVER_SYMBOL
    : ('НАКЛИЕНТЕНАСЕРВЕРЕ' | 'ATCLIENTATSERVER') -> popMode
    ;

ANNOTATION_ATCLIENT_SYMBOL
    : ('НАКЛИЕНТЕ' | 'ATCLIENT') -> popMode
    ;

ANNOTATION_ATSERVER_SYMBOL
    : ('НАСЕРВЕРЕ' | 'ATSERVER') -> popMode
    ;

ANNOTATION_BEFORE_SYMBOL
    : ('ПЕРЕД' | 'BEFORE') -> popMode
    ;
...
ANNOTATION_AROUND_SYMBOL
    : ('ВМЕСТО' | 'AROUND') -> popMode
    ;

ANNOTATION_CHANGEANDVALIDATE_SYMBOL
    : ('ИЗМЕНЕНИЕИКОНТРОЛЬ' | 'CHANGEANDVALIDATE') -> popMode
    ;
...
PREPROC_ENDDELETE
    : '#' [ \t]*
    ('КОНЕЦУДАЛЕНИЯ' | 'ENDDELETE') -> popMode, channel(PREPROC_DELETE_CHANNEL);
